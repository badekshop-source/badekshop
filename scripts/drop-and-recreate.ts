...
const dropAndRecreate = async () => {
  console.log('⚠️  Dropping existing tables and recreating with correct schema...\n');

  // Drop all tables with CASCADE using raw queries
  console.log('Dropping existing tables...');
  
  await sql`DROP TABLE IF EXISTS kyc_documents CASCADE`;
  await sql`DROP TABLE IF EXISTS reviews CASCADE`;
  await sql`DROP TABLE IF EXISTS orders CASCADE`;
  await sql`DROP TABLE IF EXISTS refund_policies CASCADE`;
  ...
  await sql`DROP TABLE IF EXISTS admin_activity_logs CASCADE`;
  await sql`DROP TABLE IF EXISTS blogs CASCADE`;
  await sql`DROP TABLE IF EXISTS blog_categories CASCADE`;
  await sql`DROP TABLE IF EXISTS discounts CASCADE`;

  // Drop custom types
  await sql`DROP TYPE IF EXISTS product_category CASCADE`;
